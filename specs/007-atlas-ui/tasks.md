...
- [x] T-02: `tokens.ts` — color / type / spacing tokens as the only source of truth; export typed constants (AC-5)
- [x] T-03: `templates/layout.ts` — mission-control layout (top nav + left rail + main + footer); `templates/paper.ts` — HTML primitives (Card, Label, Metric, Rule, Tag). **Left rail derived from `PANELS` registry (v1.4).** (AC-4, AC-14)
- [x] T-04: `server.ts` — Node 22 native http server, routing, SSR with embedded JSON-LD (AC-1, AC-4)
- [x] T-05: `tenant.ts` — tenant resolver from URL path `/t/<tenantId>` or header; reject unknown with empty-state (never another tenant's data) (AC-2, AC-8)
- [x] T-06: `panels/welcome.ts` — tenant summary + workflow counts (active / waiting human) + compliance counts (pending / missed) + last event + total events + Navigate grid to the other 10 panels (AC-7, AC-6, AC-12, AC-13)
- [x] T-07: `panels/events.ts` — raw event stream, paginated, lineage-rendered (`correlationId`, `causationId`, `actor`, `occurredAt`, `payload`) (AC-1)
- [x] T-08: `panels/activity.ts` — events sorted by `occurredAt`, grouped by day (AC-1)
- [x] T-09: `panels/logs.ts` — events with a different default filter (last N, no aggregation) (AC-1)
- [x] T-10: `panels/health.ts` — System Health panel: principal projection re-run + SHA-256 of result; alert on divergence (AC-3)
- [x] T-11: `panels/register.ts` — panel registry; each panel declares its backing model; absent if not registered (AC-6)
- [x] T-12: `cli.ts` — `node apps/atlas/src/cli.ts serve --port 8788 --tenant <id>` + `check` subcommand
- [x] T-13: Tests AC-1..AC-8 + the dedicated tenant-isolation test against two seeded tenants (AC-1..AC-8)
- [x] T-14: Static font assets in `apps/atlas/assets/fonts/`; `@font-face` declarations in templates; no CDN (AC-4)
- [x] T-15: Token-linter test (CI-blocking): fails on raw color / font / spacing literals outside `tokens.ts` (AC-5)
- [x] T-16: Evidence run: two seeded tenants; no cross-tenant rendering; 405 on writes; replay hashes match (AC-1, AC-2, AC-3, AC-7)
...
- [x] T-17: `panels/throughput.ts` — windowed projection over events (per-tenant time window) (AC-7)
- [x] T-18: `panels/monitoring.ts` — alerts panel (reads Revenue Visibility alerts)
- [x] T-19: Performance hardening: precomputed projections on disk for tenants with > 10k events (AC-7)
...
- [x] T-20: `panels/active-processes.ts` — Workflow engine backing model | live instances sorted by ageMs desc
- [x] T-21: `panels/queue-status.ts` — Event bus backing model | per-workflow counts + 24h totals
- [x] T-22: `panels/workflow-metrics.ts` — Workflow execution metrics | windowed completion/compensation + avg
- [x] T-23: `panels/compliance.ts` — Tax & Compliance obligations (Spec 004 backing model) | obligation grouping + totals + last evaluation

---

## 5. Phase 5 — BLOCKED (agent runtime + second tenant + auth)

- [ ] T-24: `panels/integrations.ts` — Integrations registry backing model | Phase 5 not started (blocked)
- [ ] T-25: Multi-tenant view (multi-pane) | Phase 5 not started (blocked)
- [ ] T-26: Real authentication | Phase 5 not started (blocked)
- [ ] T-27: Real-time SSE updates | Phase 5 not started (blocked)
