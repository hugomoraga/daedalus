...
> **Revision note (v0.2.0).** Tasks restructured to reflect:
> - `Offering` extracted to its own module (`@daedalus/offering`) — see `OF-*` tasks.
> - `Candidate` is a projection, not a first-class aggregate — tasks reflect projection semantics.
> - Channel language platform-agnostic — ChannelAdapter is the seam; LinkedIn is one adapter.
> - LLM is not wired in v0 — `LLMPort` interface is declared but no concrete adapter is shipped.
...
|---|---|---|---|
| OF-01 | `packages/offering/` scaffolding: directory, `package.json` with **no** `dependencies`, `README.md` | AC-13 | ⏸ |
| OF-02 | `domain/offering.ts` — Offering type (read-only) | AC-1, AC-13 | ⏸ |
| OF-03 | `domain/signal.ts` — Signal type (read-only) | AC-4 | ⏸ |
| OF-04 | `domain/match.ts` — Match type (read-only) | AC-4 | ⏸ |
| OF-05 | `adapters/json-schema-validator.ts` — JSON Schema for `offering.json` | AC-1 | ⏸ |
| OF-06 | `application/validate-offering.ts` — `validateOffering(jsonString): Offering | ValidationError` | AC-1 | ⏸ |
| OF-07 | `application/match-signal.ts` — `matchSignalAgainstOffering(Signal, Offering): Match | null` (deterministic, keyword-based, no LLM) | AC-4 | ⏸ |
| OF-08 | `application/deps.ts` — `OfferingDeps` | — | ⏸ |
| OF-09 | `cli.ts` — `offering:validate`, `offering:show` | AC-1 | ⏸ |
| OF-10 | Tests for the matcher (keyword scoring, negative keyword veto, threshold mapping) | AC-4 | ⏸ |
| OF-11 | Tests for the validator (well-formed, malformed, missing fields) | AC-1 | ⏸ |
| OF-12 | Public contract test (only the documented exports are reachable; ADR-004) | AC-13 | ⏸ |
...
| ID | Task | Spec AC | Status |
|---|---|---|---|
| IA-01 | `packages/inbound-agent/` scaffolding: directory, `package.json` with **no** `dependencies`, `README.md` | AC-11 | ⏸ |
| IA-02 | `domain/channel.ts` — Channel type (with `kind` discriminator); tenant-scoped; re-exports Signal and Match from `@daedalus/offering` | AC-2, AC-12, AC-13 | ⏸ |
| IA-03 | `domain/candidate.ts` — Candidate type (projection; no first-class aggregate; derived from event replay) | AC-5, AC-6 | ⏸ |
| IA-04 | `domain/run.ts` — AgentRun (workflow instance metadata) | AC-3 | ⏸ |
| IA-05 | `application/ports/channel-adapter.ts` — ChannelAdapter interface (read, OAuth refresh, summary) | AC-2 | ⏸ |
| IA-06 | `application/ports/llm-port.ts` — `LLMPort` interface (declared but no concrete adapter wired in v0) | AC-14 | ⏸ |
| IA-07 | `application/run.ts` — AgentRun lifecycle (workflow instance under `inbound-agent-run.v0.1.0`) | AC-3 | ⏸ |
| IA-08 | `application/deps.ts` — `InboundAgentDeps` | AC-3 | ⏸ |
| IA-09 | `adapters/channels/linkedin-channel-adapter.ts` — the **only** file in the agent that mentions "linkedin" (AC-12) | AC-2, AC-12 | ⏸ |
| IA-10 | `adapters/channels/linkedin-oauth-adapter.ts` — LinkedIn OAuth flow | AC-2 | ⏸ |
| IA-11 | `adapters/tenant-secrets.ts` — loads `channel-credentials.json` (gitignored) | R3 | ⏸ |
| IA-12 | `adapters/run-log.ts` — writes `.data/tenants/<t>/inbound-agent-runs.jsonl` (gitignored) | AC-10 | ⏸ |
| IA-13 | `cli.ts` — entry: `agent:connect`, `agent:disconnect`, `agent:run`, `agent:promote`, `agent:dismiss`, `agent:status` | AC-2, AC-3, AC-5, AC-6 | ⏸ |
| IA-14 | `apps/cli/src/index.ts` — wire the six agent commands | AC-2, AC-3, AC-5, AC-6 | ⏸ |
| IA-15 | `blueprints/workflows/inbound-agent-run.v0.1.0.json` — workflow artifact (Conceptual shape in Plan 009 §3) | AC-3, AC-8 | ⏸ |
| IA-16 | `config/tenants/tenant-0/offering.json` — sample offering for Tenant 0 (committed; validated by `@daedalus/offering`) | AC-1 | ⏸ |
| IA-17 | `config/tenants/tenant-0/channels.json` — public channel descriptor (no secrets) | AC-2 | ⏸ |
| IA-18 | `.gitignore` — add `config/tenants/**/channel-credentials.json` | R3 | ⏸ |
| IA-19 | `tests/agent-offering.test.ts` — AC-1 (refuses to run without offering) | AC-1 | ⏸ |
| IA-20 | `tests/agent-channel.test.ts` — AC-2 (mocked OAuth; Channel connect/disconnect events) | AC-2 | ⏸ |
| IA-21 | `tests/agent-run.test.ts` — AC-3 (full lifecycle: ready → polling → ... → awaiting_human) | AC-3 | ⏸ |
| IA-22 | `tests/agent-matching.test.ts` — AC-4 (signal with relevant keyword → CandidateSurfaced) | AC-4 | ⏸ |
| IA-23 | `tests/agent-triage-promote.test.ts` — AC-5 (CandidatePromoted → OpportunitySurfaced via Spec 003) | AC-5 | ⏸ |
| IA-24 | `tests/agent-triage-dismiss.test.ts` — AC-6 (CandidateDismissed records reason; no Opportunity) | AC-6 | ⏸ |
| IA-25 | `tests/agent-no-write-paths.test.ts` — AC-7 (greps for forbidden value-chain events / forbidden use cases) | AC-7 | ⏸ |
| IA-26 | `tests/agent-bounded.test.ts` — AC-8 (timeout → AgentRunFailed; no privilege escalation; memory bounded) | AC-8 | ⏸ |
| IA-27 | `tests/agent-tenant-isolation.test.ts` — AC-9 (two tenants' runs read only their own data) | AC-9 | ⏸ |
| IA-28 | `tests/agent-audit.test.ts` — AC-10 (replay reconstructs run state) | AC-10 | ⏸ |
| IA-29 | `tests/agent-zero-core-change.test.ts` — AC-11 (greps `packages/core/src` for forbidden new exports / new events) | AC-11 | ⏸ |
| IA-30 | `tests/agent-domain-agnostic.test.ts` — AC-12 (greps `packages/inbound-agent/src/domain/` for platform names) | AC-12 | ⏸ |
| IA-31 | `tests/agent-no-llm-adapter.test.ts` — AC-14 (verifies no concrete LLM adapter is wired; only the interface is declared) | AC-14 | ⏸ |
| IA-32 | Evidence run: end-to-end manual run + promote + dismiss against Tenant 0 seed; verify Opportunity flow | AC-3, AC-5, AC-6 | ⏸ |
...
|---|---|---|---|
| IA-33 | Multi-channel support (email, RSS, webhooks) via new ChannelAdapters | v0 shipped + per-channel ADR | ⛔ |
| IA-34 | Outbound agent actions (DMs, comments, posts) under Policy engine | Phase 3 (Policy engine) | ⛔ |
| IA-35 | Agent memory that learns from dismissals (with explicit opt-in) | v0 shipped + ADR | ⛔ |
| IA-36 | LLM-backed matching under `LLMPort` (vendor chosen separately) | v0 shipped + LLM ADR | ⛔ |
| IA-37 | Webhooks (vs polling) | v0 shipped + platform cooperation | ⛔ |
| IA-38 | Cron-style scheduling + notifications | Phase 5 | ⛔ |
| IA-39 | ATLAS panel `inbound-runs` (read-only per-tenant view of agent activity) | v0 shipped | ⛔ |
...
| ID | Task | Blocked by | Status |
|---|---|---|---|
| OF-13 | Spec 003 v1 — use `@daedalus/offering` to qualify surfaced opportunities against the offering | Spec 003 v1 ratification | ⛔ |
