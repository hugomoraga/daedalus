...
- [x] T-03: Write `tools/theia/CONVENTIONS.md` — single human-readable source of truth, with worked examples (plan §7)
...
- [x] T-05: Extend `tools/theia/src/parser/completion.ts` — add `countLegacyTableRows`; resolve canonical-first inside `parseSpecCompletion` (plan §3) (AC-1, AC-2)
- [x] T-06: Write `tools/scripts/check-spec-conventions.mjs` — fail on missing `tasks.md`, zero-checkbox `tasks.md` (non-Draft), malformed task ID, unknown Status word (plan §4) (AC-3)
- [x] T-07: Wire `check-spec-conventions.mjs` into the `npm test` chain in root `package.json`; add `lint:spec-conventions` script (plan §4)
- [x] T-08: Add Theia drift widget in `tools/theia/src/views/overview.ts` + new `conventionIssues` field in `parser/specs.ts` (plan §5) (AC-4)
- [x] T-09: Tests for the extended parser — `tools/theia/tests/completion.test.ts` (AC-1, AC-2)
- [x] T-10: Tests for the linter — `tools/scripts/tests/check-spec-conventions.test.mjs` (AC-3, all failure modes)
- [x] T-11: Tests for the drift widget data — `tools/theia/tests/views.test.ts` (AC-4)
...
- [x] T-12: Migrate `specs/001-revenue-visibility/tasks.md` — 16 ✅ rows → `- [x]`; 0 ⏸ rows
- [x] T-13: Migrate `specs/002-proposal-generation/tasks.md` — 18 boxes (10 v0 shipped + 4 v1 shipped + 4 forward-planning pending)
- [x] T-14: Migrate `specs/004-tax-compliance-guard/tasks.md` — 20 ✅
- [x] T-15: Migrate `specs/006-core-value-chain-completion/tasks.md` — 15 ✅
- [x] T-16: Migrate `specs/007-atlas-ui/tasks.md` — 27 boxes (23 shipped + 4 blocked)
- [x] T-17: Migrate `specs/008-workflow-engine/tasks.md` — 30 boxes (all pending; build authorized)
- [x] T-18: Migrate `specs/009-policy-engine/tasks.md` — 20 boxes (18 shipped + 2 intentional non-tasks)
- [x] T-19: Migrate `specs/010-authoritative-rule-source/tasks.md` — 20 boxes (19 shipped + 1 consolidated)
- [x] T-20: Migrate `specs/011-workflow-engine-projections/tasks.md` — 14 ✅
- [x] T-21: Migrate `specs/013-jurisdiction-model/tasks.md` — 20 boxes (19 ✅ + J-13 intentional non-task)
- [x] T-22: Migrate `specs/014-social-to-opportunity-mvp/tasks.md` — 55 boxes (all ⏸/⛔ → `- [ ]`)
- [x] T-23: Create `specs/003-opportunity-discovery/tasks.md` — Draft, empty checklist (linter-exempt)
- [x] T-24: Create `specs/005-administrative-shield/tasks.md` — Draft, empty checklist (linter-exempt)
